...
for (p in list_flights(DATA_DIR)) {
  ...
  bo  <- tryCatch(york_boot(x[k], y[k], 1, 0.2, blocks = du$leg_id[k]), error = function(e) list(lo = NA, hi = NA))
...
em_urban <- function(f) { d <- detect_level_legs(read_icartt(f)$data); lg <- tag_region(leg_metrics(d))
  ...
  list(x = du$CH4_ppb_enh[k], y = du$C2H6_ppb_enh[k], fit = ethane_methane_ratio(du, "CH4_ppb", "C2H6_ppb", 20)) }
